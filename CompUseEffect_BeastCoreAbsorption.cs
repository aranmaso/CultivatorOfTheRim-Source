...
public class CompUseEffect_BeastCoreAbsorption : CompUseEffect
{
    public CompProperties_UseEffectBeastCoreAbsorption Props => (CompProperties_UseEffectBeastCoreAbsorption)props;
    ...
    public override void DoEffect(Pawn pawn)
    {
        ...
        if (pawn.HaveCultivation())
            {
                Hediff_CultivationLevel level = Cultivation_Utility.FindNotMaxedCultivationLevel(pawn);
                if (level != null)
                {
                    HediffDef pawnCult = level.def;
                    CompBeastCore beastCore = parent.TryGetComp<CompBeastCore>();
                    if (beastCore != null)
                    {
                        CultivationHediffDef beastCult = beastCore.ownerCultivation;
                        int num = level.cultivationDef.realmPower;
                        int num2 = beastCult.realmPower;
                ...
                {
                    Messages.Message($"{pawn.LabelShort} has already reach the peak of current realm, the core is wasted", MessageTypeDefOf.NegativeEvent);
                }
            }
            else
            {
                ...
                {
                    Messages.Message($"{pawn.LabelShort} is a Body Cultivator and thus, unable to benefit from Qi within the Core", MessageTypeDefOf.NegativeEvent);
                }
                else
                {
                    if (Rand.Value <= 0.10f)
