...
namespace CultivatorOfTheRim
{
    public class Projectile_QiExplosion : Projectile_Explosive
    {
        public override int DamageAmount
        {
            get
            {
                if (Launcher is Pawn pawnLauncher)
                {
                    ...
                    {
                        return Mathf.RoundToInt(def.projectile.GetDamageAmount(equipment) * launcher.GetStatValue(CTR_DefOf.CultivationSpeed));
                    }
                    else
                    {
                        return 0;
        ...
        protected override void Explode()
        {
            ...
            foreach(Thing item in GenRadial.RadialDistinctThingsAround(pos,map,def.projectile.explosionRadius * 2,true).ToList())
            {
                    ...
                    if(Rand.Chance(0.5f))
                    {
                        try
                        {
                            ...
                            if (type != null)
                            {
                                ...
                                Effecter effecter = GetEffecter(type)?.Spawn(item.PositionHeld, map);
                                effecter.Cleanup();
                                GenExplosion.DoExplosion(item.PositionHeld, map, num, CTR_DefOf.CTR_Qi_Injury_Explosion, launcher, Mathf.RoundToInt(2 * num), ArmorPenetration);
                            }
                            
                        }
                        catch(Exception ex) 
                        {
                            Log.Error($"Error spawning explosion at {pos} for Thing {item}. {ex}");
                        }
                    }
                }
            }
        ...
        public float GetTypeAndColor(ThingDef input,out string type)
        {
            float num = 0f;
            string tag = "N/A";
            if (input.tradeTags.NullOrEmpty())
            {
                type = null;
                return 0f;
            }
